...
for _name, _val in options.items():
    ...
    t1 = timeit("e1 = proselint.tools.lint(_text, _cfg, _checks, _hash)", globals=globals(), number=1)
    print(f"Lint with cfg={_name} took {t1 * 1e3:4.3f} ms uncached")
    lint_cache.cache.clear()
    t2 = timeit("e1 = proselint.tools.lint(_text, _cfg, _checks, _hash)", globals=globals(), number=1)
    print(f"Lint with cfg={_name} took {t2 * 1e3:4.3f} ms uncached rerun")
    t3 = timeit("e2 = proselint.tools.lint(_text, _cfg, _checks, _hash)", globals=globals(), number=1)
