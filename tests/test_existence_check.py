...
from proselint.lint_checks import existence_check as chk
...
class TestCheck(Check):
    ...
    def test_smoke(self):
        ...
        assert chk("""abc is as easy as 123""", self.L, self.err, self.msg) != []
        assert chk("""easy breezy""", self.L, self.err, self.msg) == []
        assert (
            self.err in chk("""abc is as easy as 123""", self.L, self.err, self.msg)[0]
        )
        assert (
            self.msg in chk("""abc is as easy as 123""", self.L, self.err, self.msg)[0]
        )

    def test_multiple_matches(self):
        """Test that multiple matches are found correctly."""
        assert (
            len(chk("""abc and abc are as easy as 123""", self.L, self.err, self.msg))
            == 2
        )
        assert (
            len(
        ...
        assert chk("""abcabc are easy as 123""", self.L, self.err, self.msg) == []

    def test_string_types(self):
        """Test that the function works with different string types."""
        # TODO: can be removed, strings are unicode now, no difference in using " or '
        #       -> but could add r or other literals
    ...
    def test_exceptions(self):
        ...
        regex = [r"\b(\w+)\b\s\1"]
        no = ["should should"]
        errs = chk("should should flag flag.", regex, "", "", require_padding=False)
        assert len(errs) == 2
        errs = chk(
